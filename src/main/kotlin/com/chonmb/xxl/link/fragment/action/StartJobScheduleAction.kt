...
class StartJobScheduleAction : AStarAction {
    val render: XxlFunctionMenuRender

    constructor(render: XxlFunctionMenuRender) : super({ "开始任务" }, jobIcons.startIcon) {
        this.render = render
    }
    override fun showMsg(): String {
        return "目标环境：${globalConfig.activeConfig?.env}\n" +
                "目标域名：${globalConfig.activeConfig?.domain}\n" +
                "开启任务：${render.info.jobHandlerName}\n" +
